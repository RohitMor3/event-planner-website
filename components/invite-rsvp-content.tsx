    ...
    return <div className="mx-auto w-full max-w-2xl">
        <Card>
            ...
            <p className="text-sm text-[var(--muted-foreground)]">
                {event.eventDate ? new Date(event.eventDate).toISOString(): "No date selected"}
            ...
            <CardContent>
                {submitted? (
                    ...
                    Thanks your RSVP has been recorded (or updated)</p>
                ): null}
                <Form action={submitRsvpForToken}>
                    ...
                    <FormField>
                        ...
                        <Input id="email" name="email" required placeholder="example@mail.com"/>
                    </FormField>
                    <FormField>
                        <Label htmlFor="status">Attendance</Label>
                        <select id ="status" name="status" required defaultValue="going" className="flex h-10 w-full rounded-md border border-[var(--border) bg-[var(--surface)] px-3 py-2 text-sm text-[var(--foreground)"> 
